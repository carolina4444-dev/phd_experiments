...
A strong thesis-ready matrix would be:

Study	Variants
RL vs Random	RL, Random Search
RL Algorithm	REINFORCE, DQN, A2C, A3C
Exploration	Softmax, ε-Greedy, Entropy
Output Layer	Softmax, Gumbel
Logits Usage	Logits, Probabilities
Action Mask	On, Off
Advantage	A2C, A3C
Replay	None, Uniform, PER
Policy	Single, Dual
Encoder	MLP, Transformer, VQ-VAE, Transformer+VQ-VAE
State	Full, Reduced, Positional
Reward	Sparse, Dense, FLOPs-aware
Initialization	Random, Conv3x3, Skip
Refinement	Sequential, Single-shot
Dataset	CIFAR10, CIFAR100, ImageNet16-120
Budget	50–1000 episodes


nas_ablation_framework/
...
A2C
A3C (true multi-worker)
REINFORCE
DQN
Random Search
Dual-policy refinement
Single-policy refinement
Single-shot NAS
VQ-VAE encoder
Transformer encoder
MLP encoder
...
Advantage Actor-Critic (A2C) and Asynchronous Advantage Actor-Critic (A3C) are two popular policy gradient methods in reinforcement learning. Both algorithms utilize an actor-critic architecture, where the actor is responsible for selecting actions based on a policy, and the critic estimates the value function to provide feedback to the actor.

One additional note for the thesis: your current posted implementation is not actually A3C. It is much closer to an A2C/Advantage Actor-Critic variant because there is:

one environment,
one trajectory,
one synchronous update,
no workers,
no asynchronous gradient updates.

For your ablation matrix, I would map:

Config	Implementation
random	RandomSearchAgent
reinforce	Monte-Carlo policy gradient
a2c	Your current code (after cleanup)
a3c	Multi-process version with shared weights
dqn	Separate implementation
