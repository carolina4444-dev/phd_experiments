...
class AGNewsNASEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == self.STOP:
            ...
            if len(self.tree_encoding) >= self.max_nodes - 1:
                done = True
    ...
    def build_child_model(self):

        return build_tree_model(
            input_shape=self.problem.input_shape,
            num_classes=4,
            tree_encoding=self.tree_encoding,
            max_length=self.problem.max_length,
            vocab_size=self.problem.vocab_size,
            embedding_dim=self.problem.embedding_dim,
        )
    ...
    def evaluate_architecture(self):
        ...
        model = self.build_child_model()

        history = self.train_child_model(
            model
        )
...
def main():
    ...
    plot_model(
        final_model,
        to_file="results/final_model.png",
        show_shapes=True,
